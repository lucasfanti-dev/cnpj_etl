...
log_filename = '../logs/18_replace_optante_simples.log'
...
# Função para substituir os valores na coluna 2
@ray.remote
def process_file(file):
    ...
    try:
        ...
        for line in lines:
            ...
            if len(columns) > 1:  # Verifica se a linha tem pelo menos 2 colunas
                if columns[1] == '"S"':
                    columns[1] = '"OPTANTE"'
                elif columns[1] == '"N"':
                    columns[1] = '"NAO OPTANTE"'
...
def replace_values_in_column(estabele_folder):
    tasks = []
    for file in Path(estabele_folder).glob('*SIMPLES.CSV*'):
