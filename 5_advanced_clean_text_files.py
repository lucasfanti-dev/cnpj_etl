...
log_filename = '../logs/5_advanced_clean_text_files.log'
...
def process_file(txt_file):
    ...
    try:
        ...
        processed_lines = []
        for line in lines:
            line = line.strip() # Remove espacos em branco no inicio e no final
            line = re.sub(r'[^\x00-\x7F]+', '', line)  # Remove caracteres nao ASCII
            line = re.sub(r'[\x00-\x1F\x7F\x8d]', '', line)  # Remove caracteres de controle e 0x8d
            line = unidecode(line.upper())  # Converte para maiusculas e remove acentos
            line = line.upper()  # Converte para maiusculas e remove acentos
            line = re.sub(r'\s+', ' ', line)  # Reduz multiplos espacos para um unico espaco
            line = re.sub(r'\s*;\s*', ';', line)  # Remove espacos ao redor dos separadores
            line = line.replace('\x00', '')
            line = ';'.join(re.sub(r'\s+', ' ', part).strip() for part in line.split(';'))  # Normaliza cada parte separada por ponto e virgula
