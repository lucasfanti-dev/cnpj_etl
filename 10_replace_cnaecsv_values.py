...
log_filename = '../logs/10_replace_cnaecsv_values.log'
...
# Função para ler os dados dos arquivos CNAECSV e armazená-los em um dicionário
def read_cnaecsv_files(cnaecsv_folder):
    cnaecsv_data = {}
    for file in Path(cnaecsv_folder).glob('*cnaecsv*'):
            ...
            with open(file, 'r', encoding=encoding) as txtfile:
                ...
                for line in lines:
                    ...
                    if len(columns) > 1:  # Verifica se há pelo menos duas colunas
                        ...
                        cnaecsv_data[key] = f'{'"'+ key.replace('"', '')} - {value.replace('"', '')+'"'}'
        except Exception as e:
            logging.error(f"Erro ao ler o arquivo {file}: {e}")
    return cnaecsv_data

@ray.remote
def process_estabele_file(file, cnaecsv_data):
    ...
    try:
        ...
        with open(file, 'w', encoding=encoding, newline='') as txtfile:
            for line in lines:
                columns = line.strip().split(';')
                if len(columns) > 11:  # Verifica se a linha tem pelo menos 12 colunas
                    old_value = unidecode(columns[11].upper())
                    if old_value in cnaecsv_data:
                        columns[11] = cnaecsv_data[old_value]
...
def process_estabele_files(estabele_folder, cnaecsv_data):
    tasks = []
    for file in Path(estabele_folder).glob('*estabele*'):
        tasks.append(process_estabele_file.remote(file, cnaecsv_data))
        logging.info(f'Arquivo enviado para processamento: {file}')
    ray.get(tasks)

def main():
    cnaecsv_folder = "../3validated"  # Pasta com os arquivos CNAECSV
    estabele_folder = "../3validated"  # Pasta com os arquivos ESTABELE

    logging.info('Lendo dados dos arquivos CNAECSV...')
    cnaecsv_data = read_cnaecsv_files(cnaecsv_folder)
    logging.info('Dados dos arquivos CNAECSV lidos com sucesso.')

    logging.info('Processando arquivos ESTABELE...')
    process_estabele_files(estabele_folder, cnaecsv_data)
