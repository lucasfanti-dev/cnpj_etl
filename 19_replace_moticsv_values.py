...
log_filename = '../logs/19_replace_moticsv_values.log'
...
# Função para ler os dados dos arquivos moticsv e armazená-los em um dicionário
def read_moticsv_files(moticsv_folder):
    moticsv_data = {}
    for file in Path(moticsv_folder).glob('*moticsv*'):
            ...
            with open(file, 'r', encoding=encoding) as txtfile:
                ...
                for line in lines:
                    ...
                    if len(columns) > 1:  # Verifica se há pelo menos duas colunas
                        ...
                        moticsv_data[key] = f'{'"'+ key.replace('"', '')} - {value.replace('"', '')+'"'}'
        except Exception as e:
            logging.error(f"Erro ao ler o arquivo {file}: {e}")
    return moticsv_data

@ray.remote
def process_estabele_file(file, moticsv_data):
    ...
    try:
        ...
        with open(file, 'w', encoding=encoding, newline='') as txtfile:
            for line in lines:
                columns = line.strip().split(';')
                if len(columns) > 7:  # Verifica se a linha tem pelo menos 12 colunas
                    old_value = unidecode(columns[7].upper())
                    if old_value in moticsv_data:
                        columns[7] = moticsv_data[old_value]
...
def process_estabele_files(estabele_folder, moticsv_data):
    tasks = []
    for file in Path(estabele_folder).glob('*estabele*'):
        tasks.append(process_estabele_file.remote(file, moticsv_data))
        logging.info(f'Arquivo enviado para processamento: {file}')
    ray.get(tasks)

def main():
    moticsv_folder = "../3validated"  # Pasta com os arquivos moticsv
    estabele_folder = "../3validated"  # Pasta com os arquivos ESTABELE

    logging.info('Lendo dados dos arquivos moticsv...')
    moticsv_data = read_moticsv_files(moticsv_folder)
    logging.info('Dados dos arquivos moticsv lidos com sucesso.')

    logging.info('Processando arquivos ESTABELE...')
    process_estabele_files(estabele_folder, moticsv_data)
