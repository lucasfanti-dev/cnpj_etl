...
log_filename = '../logs/26_equalize_columns_emprecsv_estabele.log'
...
def write_intermediate_file(lines, output_folder, keyword, index):
    output_file_path = Path(output_folder) / f'column_emprecsv_estabele_intermediate_{keyword.lower()}_{index}.txt'
...
def process_files(keyword, input_folder, output_folder, max_memory=MAX_MEMORY):
    ...
    if intermediate_files:
        with open(Path(output_folder) / f'column_emprecsv_estabele_{keyword.lower()}.txt', 'w', encoding='utf-8') as final_output:
...
def main():
    input_folder = "../9unified_emprecsv_estabele"
    output_folder = "../10column_emprecsv_estabele"
