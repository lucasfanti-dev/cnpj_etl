...
log_filename = '../logs/14_replace_natureza_juridica.log'
...
# Função para ler os dados dos arquivos NATJUCSV e armazená-los em um dicionário
def read_NATJUCSV_files(NATJUCSV_folder):
    NATJUCSV_data = {}
    for file in Path(NATJUCSV_folder).glob('*NATJUCSV*'):
            ...
            with open(file, 'r', encoding=encoding) as txtfile:
                ...
                for line in lines:
                    ...
                    if len(columns) > 1:  # Verifica se há pelo menos duas colunas
                        ...
                        NATJUCSV_data[key] = f'{'"'+ key.replace('"', '')} - {value.replace('"', '')+'"'}'
        except Exception as e:
            logging.error(f"Erro ao ler o arquivo {file}: {e}")
    return NATJUCSV_data

@ray.remote
def process_estabele_file(file, NATJUCSV_data):
    ...
    try:
        ...
        with open(file, 'w', encoding=encoding, newline='') as txtfile:
            for line in lines:
                columns = line.strip().split(';')
                if len(columns) > 2:  # Verifica se a linha tem pelo menos 3 colunas
                    old_value = unidecode(columns[2].upper())
                    if old_value in NATJUCSV_data:
                        columns[2] = NATJUCSV_data[old_value]
...
def process_estabele_files(estabele_folder, NATJUCSV_data):
    tasks = []
    for file in Path(estabele_folder).glob('*EMPRECSV*'):
        logging.info(f'Arquivo enviado para processamento: {file}')
        tasks.append(process_estabele_file.remote(file, NATJUCSV_data))
    ray.get(tasks)

def main():
    NATJUCSV_folder = "../3validated"  # Pasta com os arquivos NATJUCSV
    estabele_folder = "../3validated"  # Pasta com os arquivos ESTABELE

    logging.info('Lendo dados dos arquivos NATJUCSV...')
    NATJUCSV_data = read_NATJUCSV_files(NATJUCSV_folder)
    logging.info('Dados dos arquivos NATJUCSV lidos com sucesso.')

    logging.info('Processando arquivos ESTABELE...')
    process_estabele_files(estabele_folder, NATJUCSV_data)
